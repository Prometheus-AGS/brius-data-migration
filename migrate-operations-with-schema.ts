...
async function migrateOperationsWithSchema() {
  console.log('🚀 Starting operations migration with updated schema (metadata, created_at, updated_at)...\n');
  ...
  try {
    ...
    if (totalInserted > 0) {
      ...
      console.log('💳 Square payment operations with full card details preserved in metadata JSON');
      console.log('🔗 All financial data linked to default case for relationship integrity');
    } else {
