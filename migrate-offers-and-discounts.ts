...
async function migrateOffersAndDiscounts() {
  ...
  try {
    ...
    // Get doctor mappings (we'll map by the doctor_id since legacy_doctor_id seems empty)
    const doctorMappings = new Map<number, string>();
    const doctorQuery = await targetPool.query(`
      SELECT id, doctor_id, legacy_doctor_id 
      FROM doctors 
    ...
    doctorQuery.rows.forEach(row => {
      if (row.doctor_id) {
        doctorMappings.set(row.doctor_id, row.id);
      }
      if (row.legacy_doctor_id) {
        doctorMappings.set(row.legacy_doctor_id, row.id);
      }
    });
