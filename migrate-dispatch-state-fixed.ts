...
// Valid enum values: 'submitted', 'approved', 'shipped', 'cancelled'
function mapStatusToOrderStatus(statusCode: number, isActive: boolean): string {
  // Based on analysis: Status 11 = review/approval, Status 12 = processing/production
  switch (statusCode) {
    case 11:
      return isActive ? 'submitted' : 'approved'; // submitted for review -> approved
    case 12:
      return isActive ? 'approved' : 'shipped'; // approved for production -> shipped
    default:
      return isActive ? 'submitted' : 'approved'; // fallback
  }
}

// Cache for previous states to avoid repeated queries
...
async function getPreviousStatus(instruction_id: number, changed_at: Date, sourceClient: Client): Promise<string | null> {
  const cacheKey = `${instruction_id}_${changed_at.getTime()}`;
  
  if (previousStateCache.has(cacheKey)) {
    return previousStateCache.get(cacheKey) || null;
  }
  ...
  try {
    ...
    if (prevState.rows.length > 0) {
      ...
      const status = mapStatusToOrderStatus(prev.status, prev.on);
      previousStateCache.set(cacheKey, status);
      return status;
    }
  } catch (error) {
  ...
  previousStateCache.set(cacheKey, '');
  return null;
}
