...
async function migrateCompleteTechnicians() {
  console.log('🚀 Starting complete technicians migration (CORRECTED VERSION)...');
  console.log('   Phase 1: Migrate technician users to profiles & technicians');
  console.log('   Phase 2: Migrate technician roles');
  console.log('   Schema: Using first_name/last_name, employee_id, status, legacy_technician_id');
  console.log('');
  ...
  try {
    ...
    for (let i = 0; i < technicianUsers.rows.length; i += batchSize) {
      ...
      for (const user of batch) {
        try {
          ...
          if (!existingProfile) {
            // Create profile first - CORRECTED SCHEMA
            const { data: newProfile, error: profileError } = await supabase
              .from('profiles')
              .insert({
                profile_type: 'technician',
                first_name: user.first_name || user.username, // CORRECTED: first_name not full_name
                last_name: user.last_name || '', // CORRECTED: last_name required
                email: user.email,
          ...
          if (!existingTechnician) {
            // Create technician record - CORRECTED SCHEMA
            const { data: newTechnician, error: technicianError } = await supabase
              ...
              .insert({
                ...
                email: user.email,
                employee_id: user.username, // CORRECTED: employee_id now exists
                status: user.is_active ? 'active' : 'inactive', // CORRECTED: status enum
                is_active: user.is_active,
                hire_date: user.date_joined,
                created_at: user.date_joined,
                updated_at: user.last_login || user.date_joined,
                legacy_technician_id: user.id, // CORRECTED: legacy_technician_id now exists
                legacy_user_id: user.id,
                metadata: {
                  migrationDate: new Date().toISOString(),
                  sourceTable: 'auth_user',
            ...
            techniciansCreated++;
            technicianProfiles.push({ id: profileId, employee_id: user.username });
            console.log(`   ✅ Created technician record for ${user.username}`);
          } else {
            // Add existing technician profile to our list
            technicianProfiles.push({ id: profileId, employee_id: user.username });
    ...
    console.log(`   • Total technician profiles available: ${technicianProfiles.length}`);
    ...
    // Use the first available technician profile for role assignments
    // CORRECTED: technician_roles.technician_id references profiles.id directly
    const primaryTechnicianProfile = technicianProfiles[0];
    console.log(`Using primary technician profile: ${primaryTechnicianProfile.employee_id} (${primaryTechnicianProfile.id})`);
    ...
    const transformedRoles = sourceRoles.rows.map(role => {
      ...
      return {
        technician_id: primaryTechnicianProfile.id, // CORRECTED: references profiles.id
        role_type: roleType,
        role_name: role.name,
    ...
    const { data: sampleProfiles } = await supabase
      .from('profiles')
      .select('id, first_name, last_name, username, legacy_user_id')
      .eq('profile_type', 'technician')
      .limit(5);

    const { data: sampleRoles } = await supabase
      .from('technician_roles')
      .select('role_name, role_type, abbreviation, legacy_role_id')
      .limit(5);
    ...
    if (sampleProfiles && sampleProfiles.length > 0) {
      ...
      sampleProfiles.forEach((profile, i) => {
        console.log(`   ${i + 1}. ${profile.first_name} ${profile.last_name} (${profile.username}) - Legacy: ${profile.legacy_user_id}`);
      });
    }

    if (sampleRoles && sampleRoles.length > 0) {
      console.log('\n📋 Sample technician roles:');
      sampleRoles.forEach((role, i) => {
        console.log(`   ${i + 1}. ${role.role_name} (${role.role_type}) - ${role.abbreviation} - Legacy: ${role.legacy_role_id}`);
      });
    }
