...
class TreatmentPlansDifferentialMigration {
  ...
  private async migrateTreatmentPlansBatch(
    ...
    try {
      ...
      console.log(`✅ Successfully migrated ${insertedCount} treatment plans`);
  ...
  public async executeDifferentialMigration(): Promise<TreatmentPlanMigrationStats> {
    console.log('🚀 Starting Treatment Plans Differential Migration...\n');
  ...
  public async validateMigration(): Promise<void> {
    ...
    try {
      ...
      if (sourceCount === targetMigrated) {
        ...
        console.log(`⚠️  ${sourceCount - targetMigrated} treatment plans still missing`);
      }
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log('Usage: npx ts-node migrate-treatment-plans-differential.ts [migrate|validate]');
        process.exit(1);
    }
