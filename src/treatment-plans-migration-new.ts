...
async function migrateTreatmentPlans() {
  ...
  try {
    ...
    for (let i = 0; i < treatment_plans.length; i += BATCH_SIZE) {
      ...
      if (placeholders.length > 0) {
        await targetClient.query(`
          INSERT INTO public.treatment_plans (creator_id, name, project_type, status, file_uid, file_size_bytes, is_public, created_at, legacy_plan_id)
    ...
    console.log(`⚠ Skipped ${skipped} treatment_plans (no creator mapping)`);
