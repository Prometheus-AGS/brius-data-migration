...
async function migrateFiles() {
    ...
    try {
        ...
        while (offset < totalFiles) {
            ...
            try {
                ...
                for (const file of filesToProcess) {
                    ...
                    // Add to batch - matching the correct schema
                    placeholders.push(
        ...
        // Final verification
        const finalCount = await targetClient.query('SELECT COUNT(*) as count FROM files WHERE legacy_file_id IS NOT NULL');
        console.log(`   🔍 Total migrated files in target: ${finalCount.rows[0].count}`);
