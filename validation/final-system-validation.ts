...
import { ValidationFramework } from '../src/validation/validation-framework';
...
import { TechniciansValidator } from './validate-technicians';
import { TechnicianRolesValidator } from './validate-technician-roles';
import { MessageAttachmentsValidator } from './validate-message-attachments';

export class FinalSystemValidator {
  ...
  private readonly FINAL_MIGRATION_TABLES = [
    {
      name: 'technicians',
      priority: 'high',
      validator: TechniciansValidator,
      dependencies: ['profiles']
    },
    {
      name: 'technician_roles',
      priority: 'high',
      validator: TechnicianRolesValidator,
      dependencies: ['technicians']
    },
    {
      name: 'message_attachments',
      priority: 'high',
      validator: MessageAttachmentsValidator,
  ...
  async validateFinalSystem(): Promise<SystemValidationResult> {
    ...
    try {
      ...
      const validationResults: TableMigrationResult[] = [];
      ...
      const systemResult: SystemValidationResult = {
  ...
  private async validateDatabaseSchemas(): Promise<ValidationResult> {
    const issues: ValidationIssue[] = [];
  ...
  private async validateCoreDependencies(): Promise<ValidationResult> {
    const issues: ValidationIssue[] = [];
    const targetClient = this.connectionManager.getTargetClient();
  ...
  private async validateIndividualTable(tableConfig: any): Promise<TableMigrationResult> {
    const targetClient = this.connectionManager.getTargetClient();
  ...
  private async basicTableValidation(tableName: string): Promise<ValidationResult> {
    const targetClient = this.connectionManager.getTargetClient();
    const issues: ValidationIssue[] = [];
  ...
  private async validateCrossTableRelationships(): Promise<ValidationResult> {
    const issues: ValidationIssue[] = [];
    const targetClient = this.connectionManager.getTargetClient();
  ...
  private async validateSystemPerformance(): Promise<ValidationResult> {
    ...
    try {
      ...
      const performanceChecks = await Promise.all(
        this.FINAL_MIGRATION_TABLES.map(async (table) => {
  ...
  private async validateOverallDataIntegrity(): Promise<ValidationResult> {
    const issues: ValidationIssue[] = [];
    const targetClient = this.connectionManager.getTargetClient();

    try {
      // Overall integrity checks across all migrated tables
      const totalMigratedRecords = await Promise.all(
        this.FINAL_MIGRATION_TABLES.map(async (table) => {
          const count = await this.connectionManager.getRecordCount(targetClient, table.name);
          return { table: table.name, count };
        })
  ...
  private generateSystemSummary(tableResults: TableMigrationResult[]): any {
    ...
    const totalIssues = tableResults.reduce(
      (sum, r) => sum + r.validationResult.issues.length,
      0
    );
  ...
  private generateRecommendedActions(tableResults: TableMigrationResult[]): string[] {
  ...
  private async generateFinalSystemReport(systemResult: SystemValidationResult): Promise<void> {
